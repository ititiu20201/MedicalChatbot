...
def infer_diseases(text: str, conf: InferConfig = INF_CONF) -> Dict[str, float]:
    ...
    with torch.no_grad():
        logits = MODEL(**enc)              # shape: [1, NUM_LABELS]
        probs = torch.sigmoid(logits)[0]   # shape: [NUM_LABELS]

    # Lọc theo threshold
    idxs = (probs >= conf.threshold).nonzero(as_tuple=True)[0].tolist()
    scored = [(i, float(probs[i])) for i in idxs]

    # Nếu không có nhãn vượt ngưỡng → vẫn lấy top-1 để gợi ý
    if not scored:
        top1 = int(torch.argmax(probs).item())
        scored = [(top1, float(probs[top1]))]

    # Sắp xếp giảm dần & lấy top_k
    scored.sort(key=lambda x: x[1], reverse=True)
    scored = scored[:conf.top_k]

    return {ID2LABEL[i]: round(score, 4) for i, score in scored}
...
app = FastAPI(title="Medical Chatbot - PhoBERT Inference (Chặng 1)")
