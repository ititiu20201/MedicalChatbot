...
def chat(body: ChatIn, db: Session = Depends(get_db)):
    ...
    if parsed.next_action == "call_phobert" and not missing:
        ...
        record = PatientRecord(
            ...
            user_id=user_id,
            symptoms=json.dumps(filled, ensure_ascii=False),
...
def get_all_patients(db: Session = Depends(get_db)):
    rows = db.query(PatientRecord).all()
    return [
        {
...
def export_excel(db: Session = Depends(get_db)):
    ...
    for r in rows:
        data.append({
            ...
            "User ID": r.user_id,
            "Symptoms": r.symptoms,
            "Diseases": r.predicted_diseases,
            "Department": r.recommended_department,
        })
    df = pd.DataFrame(data)
    out_path = "/mnt/data/patient_records.xlsx"
    df.to_excel(out_path, index=False)
    return FileResponse(out_path, filename="patient_records.xlsx")
