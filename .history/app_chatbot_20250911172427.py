...
def recommend_department(predicted: Dict[str, float]) -> str:
    ...
    # 2) Chạy qua các luật regex với tên bệnh
...
def chat(body: ChatIn):
    """
    Điều phối hội thoại:
    - Gọi Gemini để lấy assistant_message + slots_extracted + next_action
    - Merge slots vào state theo user_id
    - Nếu next_action=call_phobert và đủ slot -> gọi PhoBERT & map department
    """
    ...
    # 2) Gọi LLM (Gemini) theo schema JSON bắt buộc
    llm_out = call_gemini(message, state)     # dict
    parsed = LLMOutput(**llm_out)             # validate

    # 3) Merge các slot vừa rút trích
    filled = merge_slots(user_id, parsed.slots_extracted)

    # 4) Tính slot còn thiếu
    missing = [s for s in REQUIRED_SLOTS if not filled.get(s)]

    # 5) Nhánh rẽ: đủ slot thì gọi PhoBERT
    if parsed.next_action == "call_phobert" and not missing:
        ...
        dept = recommend_department(preds)
        assist_msg = (
            f"Kết quả sơ bộ: {', '.join([f'{k} ({v})' for k, v in preds.items()])}. "
            f"Khoa gợi ý: {dept}."
        )
